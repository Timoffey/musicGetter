...
class MG_Config
{
	...
	public function set($db_url, $db_login, $db_pass, $db_name, $refresh_quantity, $refresh_rate, $refresh_period){
		$refresh_rate=$this->convert_to_seconds($refresh_rate, $refresh_period);
		global $wpdb;
		$table_name = $wpdb->prefix . 'mg_config';
		$query = "UPDATE $table_name SET db_url = %s, db_login = %s, db_pass = %s, db_name=%s,refresh_rate = %d, refresh_quantity = %d";
		$wpdb->query($wpdb->prepare($query, $db_url, $db_login, $db_pass, $db_name, $refresh_rate, $refresh_quantity));
	}
